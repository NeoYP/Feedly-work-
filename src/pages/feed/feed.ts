...
//import { ImagePicker, ImagePickerOptions, ImagePickerOriginal } from '@ionic-native/image-picker';
...
export class FeedPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, private loadingCtrl: LoadingController, private toastCtrl: ToastController, private camera:  Camera, private http: HttpClient, private actionSheetCtrl: ActionSheetController, private alertCtrl: AlertController, /*private imagePicker: ImagePickerOriginal */ private modalCtrl: ModalController, private firebaseCordova: Firebase) {
    this.getPosts();
  ...
  like(post) {
    ...
    this.http.post("https://us-central1-feedly-32a42.cloudfunctions.net/updateLikesCount", JSON.stringify(body), {
      responseType: "text"
    }).subscribe((data) => {
      console.log(data)
  ...
  delete(post) {
    console.log(post.id);
      ...
      firebase.firestore().collection("posts").doc(post.id).delete().then(function() {
        console.log("Posts has deleted successfully!");
